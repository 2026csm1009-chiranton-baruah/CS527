...
/*
 * Lab 6: execution frequency per generated byte-code instruction.
 * PC is byte-addressed, so instruction index is PC / 4.
 */
static uint64_t execution_count[NP][INSTRUCTION_SIZE / 4];
...
if (opcode >= 0x18 &&
    ...
    int32_t value = registers_file[proc_id][src1];
    ...
    registers_file[proc_id][dest] = (int32_t)result;
    AC[proc_id] = registers_file[proc_id][dest];
    return advance_pc(proc_id);
}
 ...
 *
 * 30 AND / 31 OR / 32 XOR   register-register
 * 33 ANDI / 34 ORI / 35 XORI immediate
 * 36 NOT                   unary
 * ========================================================
 */
...
if (opcode >= 0x30 &&
    ...
    int32_t a = registers_file[proc_id][src1];
    int32_t result;
    ...
    if (opcode <= 0x32) {
        ...
        int32_t b = registers_file[proc_id][src2];
        if (opcode == 0x30)
        ...
        uint32_t imm = (uint32_t)src2;
        if (opcode == 0x33)
            result = (int32_t)((uint32_t)a & imm);
        else if (opcode == 0x34)
            result = (int32_t)((uint32_t)a | imm);
        else
            result = (int32_t)((uint32_t)a ^ imm);
...
// ```
    ...
    memset(execution_count[p],
           0,
           sizeof(execution_count[p]));
...
memset(execution_count[proc_id],
       0,
       sizeof(execution_count[proc_id]));
...
for (int executed = 0;
    ...
    /* Count the instruction at the PC before executing it. */
    int instruction_index = PC[proc_id] / 4;
    if (instruction_index >= 0 &&
        instruction_index < (INSTRUCTION_SIZE / 4)) {
        execution_count[proc_id][instruction_index]++;
    }
...
/*
 * Return the number of times a generated byte-code instruction
 * has executed on a processor.  The OS uses this to translate
 * processor-level counts back to source statements.
 */
                                         ...
                                         int instruction_index)
{
    ...
    return execution_count[proc_id][instruction_index];
}
