...
static uint32_t read32(int p, uint32_t addr) {
    ...
    return (uint32_t)Data[p][addr] |
           ((uint32_t)Data[p][addr + 1] << 8) |
           ((uint32_t)Data[p][addr + 2] << 16) |
           ((uint32_t)Data[p][addr + 3] << 24);
}

static void write32(int p, uint32_t addr, uint32_t val) {
    if (addr + 3 >= 4096) return;
    Data[p][addr]     = val & 0xFF;
    Data[p][addr + 1] = (val >> 8) & 0xFF;
    Data[p][addr + 2] = (val >> 16) & 0xFF;
    Data[p][addr + 3] = (val >> 24) & 0xFF;
}
...
void fetch(int p) {
    opcode[p] = Instruction[p][PC[p]];
    dest[p]   = Instruction[p][PC[p] + 1];
    src1[p]   = Instruction[p][PC[p] + 2];
    src2[p]   = Instruction[p][PC[p] + 3];
}
