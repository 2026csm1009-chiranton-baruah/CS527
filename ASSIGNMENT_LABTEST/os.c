                       ...
                       const char *data_source)
{
    ...
    /*
     * Bounded copies with guaranteed null termination.
     *
     * snprintf() avoids GCC's -Wstringop-truncation warning
     * that was produced by the previous strncpy() pattern.
     */
...
static void finish_task(Task *task)
{
    ...
    if (processor >= 0 &&
        ...
        /* Flush all user-program output before this processor is reset. */
        processor_flush_output(processor);
...
static void shutdown_os(void)
{
    ...
    /*
     * Report translation and cache activity before finalization
     * clears them.
     */
    tlb_print_stats();
    cache_print_stats();
