...
static int allocate_page_frame(int *victim_proc_out, int *victim_page_out)
{
    ...
    if (valid_proc(victim_proc) && valid_logical_page(victim_page)) {
        if (pageDirty[victim_proc][victim_page]) {
            if (swap_write_page(victim_proc,
                                victim_page,
...
int page_out(int proc_id, int logical_page)
{
    ...
    /* Ensure dirty L1/L2 lines have reached RAM before RAM is used as the
     * source buffer for the disk-backed swap page. */
    cache_writeback_frame(frame);

    if (pageDirty[proc_id][logical_page] &&
        swap_write_page(proc_id,
                        logical_page,
                        &memory[frame * PAGESIZE]) != 0)
        return -1;
                        ...
                        int address)
{
    ...
    int frame = tlb_lookup(proc_id, logical_page);
    if (frame < 0)
        frame = page_in(proc_id, logical_page);
    if (frame <= 0)
        return -1;
                      ...
                      uint8_t *value)
{
    ...
    return cache_read(proc_id, isFetch, physical, value);
}
                       ...
                       uint8_t value)
{
    ...
    return cache_write(proc_id, physical, value);
}
                              ...
                              const uint8_t *bytes)
{
    ...
    int block = swap_allocate(proc_id, logical_page);
    if (block < 0)
        return -1;

    if (swap_write_page(proc_id, logical_page, bytes) != 0) {
...
void release_process_memory(int proc_id)
{
    ...
    for (int page = 0; page < NUM_LOGICAL_PAGES; page++) {
        ...
        if (frame > 0 && valid_frame(frame)) {
            /* Dirty data may live in private L1 or shared L2. Bring it back to
             * RAM before writing the backing page to disk. */
            cache_writeback_frame(frame);

            if (pageDirty[proc_id][page] && block >= 0)
                swap_write_page(proc_id, page, &memory[frame * PAGESIZE]);
