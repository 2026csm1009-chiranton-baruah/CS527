...
void compile(const char *filename) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (strncmp(s, "Print ", 6) == 0 || strncmp(s, "print ", 6) == 0) {
            char reg[64];
            if (sscanf(s + 6, "%63s", reg) == 1 && reg[0] == 'x') {
                emit(out, 0x08, 0, 0, regnum(reg));
                pc++;
            }
            continue;
        }
        ...
        if (L[0] == '[' && R[0] == 'v') {
            ...
            emit(out, 0x26, regnum(addr), vregnum(R), 0);
            pc++;
            continue;
        }
        ...
        if (L[0] == '[') {
            ...
            emit(out, 0x06, regnum(addr), regnum(R), 0);
            pc++;
            continue;
        }
        ...
        if (L[0] == 'v') {
            ...
            if (R[0] == '[') {
                ...
                if (is_number(addr)) {
                    ...
                    emit(out, 0x25, d, 255, 0);
                    pc += 2;
                } else {
                    emit(out, 0x25, d, regnum(addr), 0);
                    pc++;
                }
            ...
            if (sscanf(R, "%s %c %s", a, &op, b) == 3) {
                ...
                if (b[0] == 'x') {
                    ...
                    emit(out, 0x0B, 254, xr, 0); /* helper move */
                    emit(out, 0x2B, d, va, xr);
                    pc += 2;
                    continue;
                }
            }
