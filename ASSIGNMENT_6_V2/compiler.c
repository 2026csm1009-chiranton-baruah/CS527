...
/*
 * Lab 6 instrumentation metadata.
 *
 * During PASS 2 every generated byte-code instruction is linked
 * to the source statement that caused it to be emitted.  The
 * processor later counts execution by byte-code PC, while the OS
 * uses this metadata to produce the source-level frequency file.
 */
static FILE *mapping_out = NULL;
static int mapping_pc = -1;
static int mapping_source_line = 0;
static char mapping_source_text[MAX_LINE_LENGTH];
...
/* The current source context is disabled for the final HALT. */
if (mapping_out != NULL &&
    mapping_pc >= 0 &&
    mapping_source_line > 0) {
    fprintf(mapping_out,
            "%d\t%d\t%s\n",
            mapping_pc,
            mapping_source_line,
 ...
 */
{
    ...
    if (sscanf(s, "%15s %63s %63s",
               move_op, move_dest, move_src) == 3 &&
        (!strcmp(move_op, "MV") ||
         !strcmp(move_op, "MOVE"))) {

        if (move_dest[strlen(move_dest) - 1] == ',')
            move_dest[strlen(move_dest) - 1] = '\0';

        if (!is_valid_register(move_dest) ||
            !is_valid_register(move_src)) {
            compiler_error(
                line_number,
                "MV/MOVE requires two integer registers");
            return -1;
        }
...
/*
 * Register-to-register assignment is a MOVE.
 */
if (is_valid_register(R))
    return 1;
...
/*
 * One metadata record is generated for every emitted source-level
 * instruction.  A statement which expands to two byte-code
 * instructions gets two records with the same source line.
 */
mapping_out = fopen("program.map", "w");

if (!mapping_out) {
    fprintf(stderr,
            "Compiler error: cannot create program.map\n");
    fclose(out);
    exit(EXIT_FAILURE);
}
...
for (int line_number = 0;
    ...
    /*
     * Preserve the original source statement (including its
     * comment) for the Lab 6 frequency report.
     */
    {
        char source_copy[MAX_LINE_LENGTH];
        strncpy(source_copy,
                lines[line_number],
                sizeof(source_copy) - 1);
        source_copy[sizeof(source_copy) - 1] = '\0';

        char *source_text = trim(source_copy);
        strncpy(mapping_source_text,
                source_text,
                sizeof(mapping_source_text) - 1);
        mapping_source_text[sizeof(mapping_source_text) - 1] = '\0';
        mapping_source_line = line_number + 1;
        mapping_pc = pc;
    }
...
/* The compiler-generated HALT is not a source statement. */
mapping_pc = -1;
mapping_source_line = 0;
